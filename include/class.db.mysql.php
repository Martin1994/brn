...
class chlorodb_mysql implements IChloroDB
{
	...
	{
		if($persistent){
			$this->db_m = mysql_pconnect($host_m, $user, $pass);
		}else{
			$this->db_m = mysql_connect($host_m, $user, $pass);
		}
		mysql_select_db($name, $this->db_m) or throw_error(mysql_error($this->db_m));
		
		if(false === $host_s){
			$this->db_s = $this->db_m;
		}else{
			if($persistent){
				$this->db_s = mysql_pconnect($host_s, $user, $pass);
			}else{
				$this->db_s = mysql_connect($host_s, $user, $pass);
			}
			mysql_select_db($name, $this->db_s) or throw_error(mysql_error($this->db_s));
			mysql_set_charset('utf8', $this->db_s);
		}
		
		mysql_set_charset('utf8', $this->db_m);
	...
	public function batch_update($table, array $data, $matrix = true)
	{
		...
		if($matrix){
			...
			foreach($data as $row){
				...
				foreach($row as $key => $value){
					if($key === '_id'){
						...
						$values .= '"'.$this->parse_text(json_encode($value, $this->json_flag)).'",';
					}else if(gettype($value) === 'string'){
						$values .= '"'.$this->parse_text($value).'",';
					}else{
						$values .= $value.',';
					}
				}
			...
			return $this->query(rtrim($qry, ',').';', $db);
		}else{
			foreach($data as $value){
				$this->update($table, $value, array('_id' => $value['_id']));
			}
			return;
		}
	}
	...
	public function create_table($table, $column)
	{
		...
		$column = $this->parse_column($column, true);
	...
	public function count($table, $where = false)
	{
		...
		$qry = 'SELECT COUNT(*) FROM '.$this->parse_table($table);
		
		if($where){
			$qry .= ' WHERE '.$this->parse_where($where);
		}
		
		$qry .= ';';
		
		$result = $this->query($qry, $db);
	...
	protected function query($query_string, $db = false)
	{
		$this->query_time += 1;
		...
		//echo $query_string,"\n";
		$result = mysql_query($query_string, $db) or throw_error(mysql_error($db).' | Query string: '.$query_string);
		if($result === true){
			return true;
		}
		if($result === false){
	...
	protected function parse_data($data)
	{
		...
		foreach($data as $key => $value){
			...
			if(gettype($value) === 'string'){
				$result .= '"'.$this->parse_text($value).'"';
			}else if(true === is_array($value)){
				$result .= '"'.$this->parse_text(json_encode($value, $this->json_flag)).'"';
			}else{
				$result .= strval($value);
			}
			$result .= ',';
				...
				default:
					if(false === is_array($value)){
						...
						if(gettype($value) === 'string'){
							...
							$result .= $this->parse_text(strval($value));
						}
					}else{
								...
								case '$in':
									...
									foreach($subvalue as $subsubvalue){
										if(gettype($subsubvalue) === 'string'){
											$result .= '"'.$this->parse_text($subsubvalue).'",';
										}else{
											$result .= $this->parse_text(strval($subsubvalue)).',';
										}
									}
